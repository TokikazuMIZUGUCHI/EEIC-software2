...
void read_object_from_file(FILE *fp, Object objs[], const size_t numobj, const Condition cond){
    double c;
    char str[1000];
    double f[100][100];
    int i = 0;
    ...
    fgets(str, 1000, fp);
    //for (i = 0; i < numobj;){
        while( (c = fscanf(fp, "%lf %lf %lf %lf %lf %s %s", &f[i][0], &f[i][1], &f[i][2], &f[i][3], &f[i][4], str, str)) != EOF ){
            i++;
            if(i == numobj){
                break;
            }
...
int main(int argc, char **argv) {
    ...
    fp = fopen(argv[1], "r");
    read_object_from_file(fp, objects, objnum, cond);
    
    // objects[1] は巨大な物体を画面外に... 地球のようなものを想定
    //objects[0] = (Object){ .m = 60.0, .x = 0, .y = -19.9, .vx = 2.0, .vy = 2.0, .prev_vx = 2.0, .prev_vy = 2.0,};
    //objects[1] = (Object){ .m = 100000.0, .x = 0, .y = 10000.0, .vx = 0.0, .vy = 0.0, .prev_vx = 0.0, .prev_vy = 0.0, };
    //objects[2] = (Object){ .m = 60.0, .x = 10, .y = -19.9, .vx = 2.0, .vy = 2.0, .prev_vx = 2.0, .prev_vy = 2.0,};
