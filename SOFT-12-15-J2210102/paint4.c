...
Canvas *init_canvas(int width, int height, char pen, char color);
...
typedef enum res{ EXIT, LINE, REC, CIRCLE, STAR, FILL, UNDO, SAVE, LOAD, CHPEN, CHCOL, UNKNOWN, ERRNONINT, ERRLACKARGS, NOCOMMAND} Result;
...
void load(Canvas *c, const char *file, History *his);
void chpen(Canvas *c,History *his, const char *s);
void chcol(Canvas *c, History *his, const char *s);
...
void draw_fillrectangle(Canvas *c, const int x0, const int y0, const int x1, const int y1){
    const int width = c->width;
    const int height = c->height;
    for (int i = 0; i < width; i++){
        draw_line(c, x0 + i, y0, x0 + i, y1);
    }
}
...
void draw_circle(Canvas *c, const int x0, const int y0, const int r){
    ...
    for (i = 0; i < width; i ++) {
        for (j = 0; j < height; j ++) {
            distance = (pow(x0 - i, 2) + pow(y0 - j, 2));
            if (distance <= pow(r, 2) && distance > pow(r - 1, 2)) {
                c -> canvas[i][j] = pen;
            }
        }
    }
...
void draw_fillcircle(Canvas *c, const int x0, const int y0, const int r){
    for (int i = r; i > 0; i--){
        draw_circle(c,x0,y0,i);
    }
}

void draw_star(Canvas *c, const int x0, const int y0){
    const int width = c->width;
    const int height = c->height;
    char pen = c -> pen;
    c -> canvas[x0][y0 - 5] = pen;
    c -> canvas[x0-1][y0 - 4] = pen;
    c -> canvas[x0][y0 - 4] = pen;
    c -> canvas[x0 + 1][y0 - 4] = pen;
    c -> canvas[x0 - 2][y0 - 3] = pen;
    c -> canvas[x0 - 1][y0 - 3] = pen;
    c -> canvas[x0][y0 - 3] = pen;
    c -> canvas[x0 + 1][y0 - 3] = pen;
    c -> canvas[x0 + 2][y0 - 3] = pen;
    c -> canvas[x0 - 5][y0 - 2] = pen;
    c -> canvas[x0 - 4][y0 - 2] = pen;
    c -> canvas[x0 - 3][y0 - 2] = pen;
    c -> canvas[x0 - 2][y0 - 2] = pen;
    c -> canvas[x0 - 1][y0 - 2] = pen;
    c -> canvas[x0][y0 - 2] = pen;
    c -> canvas[x0 + 1][y0 - 2] = pen;
    c -> canvas[x0 + 2][y0 - 2] = pen;
    c -> canvas[x0 + 3][y0 - 2] = pen;
    c -> canvas[x0 + 4][y0 - 2] = pen;
    c -> canvas[x0 + 5][y0 - 2] = pen;
    c -> canvas[x0 - 4][y0 - 1] = pen;
    c -> canvas[x0 - 3][y0 - 1] = pen;
    c -> canvas[x0 - 2][y0 - 1] = pen;
    c -> canvas[x0 - 1][y0 - 1] = pen;
    c -> canvas[x0][y0 - 1] = pen;
    c -> canvas[x0 + 1][y0 - 1] = pen;
    c -> canvas[x0 + 2][y0 - 1] = pen;
    c -> canvas[x0 + 3][y0 - 1] = pen;
    c -> canvas[x0 + 4][y0 - 1] = pen;
    c -> canvas[x0 - 3][y0] = pen;
    c -> canvas[x0 - 2][y0] = pen;
    c -> canvas[x0 - 1][y0] = pen;
    c -> canvas[x0][y0] = pen;
    c -> canvas[x0 + 1][y0] = pen;
    c -> canvas[x0 + 2][y0] = pen;
    c -> canvas[x0 + 3][y0] = pen;
    ...
    c -> canvas[x0 + 2][y0 + 2] = pen;
    c -> canvas[x0 + 3][y0 + 2] = pen;
    c -> canvas[x0 - 4][y0 + 3] = pen;
    c -> canvas[x0 - 3][y0 + 3] = pen;
    c -> canvas[x0 - 2][y0 + 3] = pen;
    c -> canvas[x0 + 2][y0 + 3] = pen;
    c -> canvas[x0 + 3][y0 + 3] = pen;
    c -> canvas[x0 + 4][y0 + 3] = pen;
    c -> canvas[x0 - 4][y0 + 4] = pen;
    c -> canvas[x0 - 3][y0 + 4] = pen;
    c -> canvas[x0 + 3][y0 + 4] = pen;
    c -> canvas[x0 + 4][y0 + 4] = pen;
}
...
void load(Canvas *c, const char *file, History *his){
    ...
    init_canvas(width, height, pen, color);
    while (fgets(command, bufsize, fp) != NULL) {
        interpret_command(command, his, c);
...
void chpen(Canvas *c, History *his, const char *s){
    ...
    char col = c -> color;
    c -> pen = *s;
    init_canvas(width, height, *s, col);
}
...
void chcol(Canvas *c, History *his, const char *s){
    ...
    c -> color = col;
    init_canvas(width, height, pen, col);
}
...
Result interpret_command(const char *command, History *his, Canvas *c)
{
    ...
    if (strcmp(s, "star") == 0) {
	int p[2] = {0}; // p[0]: x0, p[1]: y0, p[2]: x1, p[3]: x1 
	char *b[2];
	for (int i = 0 ; i < 2; i++){
	    b[i] = strtok(NULL, " ");
	    if (b[i] == NULL){
	...
	draw_fillrectangle(c,p[0],p[1],p[2],p[3]);
	return FILL;
    }
    
    if (strcmp(s, "fcircle") == 0){
        int p[3] = {0}; // p[0]: x0, p[1]: y0, p[2]: x1, p[3]: x1 
	    char *b[3];
	    for (int i = 0 ; i < 3; i++){
	        b[i] = strtok(NULL, " ");
